...
✔ SBERT는 설명 텍스트의 “의미·감정·뉘앙스”를 이해한다.
✔ IsolationForest는 전체 텍스트 분포에서 ‘이상한’ 문장들을 자동으로 골라낸다.
✔ 과장·유혹·압박 문장 패턴은 정상 설명 텍스트와 매우 다르기 때문에 이상치로 분류된다.
✔ 그래서 text_anomaly_score가 심리적 압박 문구가 들어간 매물에서 높게 나오는 것이 가능하다.
